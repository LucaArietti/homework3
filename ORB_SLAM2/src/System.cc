...
// costruisco una funzione che mi salvi i punti della mappa, usando come modello la funzione qui sopra
void System::SavePointsOfMap(const string &filename)
{
    cout << endl << "Saving points of map in " << filename << " ..." << endl;
    ...
    for (auto& elem: pointsOfMap){
        ...
        // il setprecision() serve a Sets the decimal precision to be used to format floating-point values on output operations.
        f << setprecision(9) << positionOfMapPoint.at<float>(0) << " " << positionOfMapPoint.at<float>(1) << " " << positionOfMapPoint.at<float>(2) << endl;
        
    }

    f.close();
    cout << endl << "points of the map saved!" << endl;
}
